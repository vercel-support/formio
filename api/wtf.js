...
const { FORMS_API_URL = 'https://sfgov-forms.vercel.app' } = process.env;
    ...
    if (isSFGovURL(parsed)) {
      ...
      if (parts[0] === 'project' && parts[2] === 'form' && parts[3]) {
        ...
        const [form] = await fetch(
          `http://sfds.form.io/form?_id__eq=${formId}`
        ).then((res) => res.json());
        if (form) {
...
function isDataSourceURL(url) {
  return url.hostname.endsWith('.form.io') && url.hostname.includes('sfds');
}
